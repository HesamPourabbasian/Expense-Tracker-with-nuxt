...
// Auto-detect bank from account name if not manually selected
watch(() => form.name, (newName) => {
  if (!isManuallySelected.value && newName) {
    const detected = detectBankIcon(newName)
    if (detected) {
      form.icon = detected
    }
  }
      ...
      <div class="flex items-center justify-between mb-5">
        <div class="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p class="text-xs text-slate-400">اطلاعات حساب یا کارت بانکی خود را وارد کنید.</p>
          </div>
        </div>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder="مثال: بانک سامان / بلوبانک / حساب اصلی"
            spellcheck="false"
            autocomplete="off"
            autofocus
          />
        ...
        <div>
          <div class="flex items-center justify-between mb-2">
            <label class="text-xs font-bold text-slate-700 dark:text-slate-300">انتخاب بانک یا نماد حساب</label>
          ...
          <!-- Icons Grid -->
          <div class="grid grid-cols-3 sm:grid-cols-4 gap-2 max-h-56 overflow-y-auto p-0.5 rounded-xl border border-slate-100 dark:border-slate-800/80 bg-slate-50/50 dark:bg-slate-900/40 p-2">
            <button
